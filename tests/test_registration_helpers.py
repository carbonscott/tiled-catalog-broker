...
def test_verify_dataset_with_no_entities(capsys):
    """Dataset exists but is empty: returns cleanly without trying to enter."""
    ds = MagicMock()
    ds.metadata = {"label": "DS"}
    ds.keys.return_value = []
    root = MagicMock()
    root.keys.return_value = ["DS"]
    root.__getitem__.side_effect = lambda k: ds
    verify_registration_http(root)
    out = capsys.readouterr().out
    assert "entity containers: 0" in out
