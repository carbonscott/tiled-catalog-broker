...
def validate(cfg, model_path=None):
    ...
    # `label` is the human-readable identifier; `key` is derived from it
    # (slug(label)) at registration time and written back to the YAML.
    if not cfg.get("label"):
        errors.append("'label' is required (e.g., 'Broad Sigma')")
